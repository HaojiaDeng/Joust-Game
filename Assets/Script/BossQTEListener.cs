            ...
            if (qteManager != null)
            {
                ...
                if (result != null && result.totalInputs > 0)
                {
                    ...
                    if (qteCompleted && (lastResult == null || lastResult.successfulInputs != result.successfulInputs || lastResult.damageMultiplier != result.damageMultiplier))
                    {
                        ...
                        if (!hasCalculatedDamage && BossBattle.Instance != null && BossBattle.Instance.IsBossBattleActive())
                        {
                            Debug.Log($"BossQTEListener: QTE COMPLETE! Calculating boss damage... Multiplier: {result.damageMultiplier}");
                            BossBattle.Instance.CalculateBossDamage(result.damageMultiplier);
