    ...
    public override IEnumerator Execute(QTEPattern pattern, QTEResult result, QTEUIController ui)
    {
        ...
        foreach (QTEInput input in pattern.inputSequence)
        {
            ...
            if (!isHolding)
            {
                ...
                Debug.Log("QTE FAILED - Hold failed! Immediate failure.");
            ...
            while (!released && innerRingProgress <= 1.15f)
            {
                ...
                if (Input.GetKeyUp(input.requiredKey))
                {
                    ...
                    {
                        // Released too early - QTE fails immediately
                        result.successfulInputs = 0;
                        result.isPerfect = false;
                        result.damageMultiplier = 0f;
                        result.accuracyPercent = 0f;
                        ...
                        if (GameLoopManager.Instance != null)
                        {
                            GameLoopManager.Instance.ShowCenterMessage("TOO EARLY! QTE FAILED!", 1f);
                        }
                        
                        Debug.Log("QTE FAILED - Released too early! Immediate failure.");
                        yield break; // Exit immediately
                    }
                    else
                    {
                        // Released too late - QTE fails immediately
                        result.successfulInputs = 0;
                        result.isPerfect = false;
                        result.damageMultiplier = 0f;
                        result.accuracyPercent = 0f;
                        ...
                        if (GameLoopManager.Instance != null)
                        {
                            GameLoopManager.Instance.ShowCenterMessage("TOO LATE! QTE FAILED!", 1f);
                        }
                        
                        Debug.Log("QTE FAILED - Released too late! Immediate failure.");
                        yield break; // Exit immediately
                    }
                }
            ...
            if (!released)
            {
                // Never released - QTE fails immediately
                showUI = false;
                result.successfulInputs = 0;
                result.isPerfect = false;
                result.damageMultiplier = 0f;
                ...
                Debug.Log("QTE FAILED - Never released! Immediate failure.");
                yield break; // Exit immediately
            }
