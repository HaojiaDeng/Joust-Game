...
public class ButtonMashQTE : BaseQTE
{
    ...
    public override IEnumerator Execute(QTEPattern pattern, QTEResult result, QTEUIController ui)
    {
        ...
        // If failed, mark as complete failure and exit
        if (currentPresses < targetPresses)
        {
            ...
            {
                GameLoopManager.Instance.ShowCenterMessage("FAILED! QTE FAILED!", 1f);
            }
            Debug.Log("QTE FAILED - Button mash failed! Immediate failure.");
        }
        else
        {
