...
public class QTEUIController : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (promptPanel == null)
        {
            if (gameObject.name == "PromptPanel" || gameObject.name.Contains("Prompt"))
            {
                promptPanel = gameObject;
            }
            else
            {
                promptPanel = transform.parent != null && transform.parent.name.Contains("Prompt") 
        ...
        if (promptText == null)
        {
            ...
            if (promptText == null && transform.parent != null)
            {
                promptText = transform.parent.GetComponentInChildren<TextMeshProUGUI>(true);
            }
        }
        ...
        if (promptImage == null)
        {
            ...
            if (promptImage == null && transform.parent != null)
            {
                promptImage = transform.parent.GetComponentInChildren<Image>(true);
            }
        }

        if (promptPanel != null && promptText != null)
        {
            Debug.Log($"QTEUIController auto-setup: Panel={promptPanel.name}, Text={promptText.name}");
        }
    }
